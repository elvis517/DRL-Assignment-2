...
class Game2048Env(gym.Env):
    ...
    def get_afterstate(self, board, action):
        """
        根據當前 board 與動作，計算 afterstate（玩家移動後但未加入隨機新方塊的狀態）。
        
        Args:
            board: 當前狀態（numpy array）。
            action: 動作編號（0: up, 1: down, 2: left, 3: right）。
            
        Returns:
            afterstate: 執行動作後的 board 狀態，不包含隨機新增的方塊。
        """
        board_copy = board.copy()

        # 本地版本的 compress 與 merge，不改變分數
